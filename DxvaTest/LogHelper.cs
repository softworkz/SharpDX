...
namespace MediaBrowser.Model.Logging
{
    ...
    public static class LogHelper
    {
        ...
        public static StringBuilder GetLogMessage(Exception exception)
        {
            ...
            messageText.AppendFormat("{0}: {1}{2}", exception.GetType(), exception.Message, Environment.NewLine);
        ...
        {
            messageText.AppendFormat("InnerException: {0}: {1}{2}", e.GetType(), e.Message, Environment.NewLine);
        ...
        {
            messageText.AppendFormat("Source: {0}{1}", e.Source, Environment.NewLine);
            messageText.AppendFormat("TargetSite: {0}{1}", e.TargetSite, Environment.NewLine);

            foreach (var key in e.Data.Keys)
            {
                messageText.AppendFormat("{0}: {1}{2}", key, e.Data[key], Environment.NewLine);
            }
        }
    }
}
